...
console.groupCollapsed('문자 데이터(숫자 + 문자를 포함하는 문자) - 숫자 데이터로 변경되는 사례');
...
// window.parseInt(단위를 포함하는 문자, 10(진수))   <= 단위를 제거하고 정수 값 반환
...
// window.parseFloat(단위를 포함하는 문자, 10(진수)) <= 단위를 제거하고 실수 값 반환
var convert_news_font_size_to_float = window.parseFloat(news_font_size, 10); // 16.6345 
...
console.group('데이터 -> 불리언 데이터로 변경되는 사례')
...
console.log('Boolean(""):', Boolean(""));   // 공백문자 " " 은 false 로 변경 
...
console.log('!!str:', !!str);  
console.log('!!"":', !!"");     // 공백문자 " " 은 false 로 변경 
console.log('!!" ":', !!" ");   // " " 은 true 로 변경   
console.log('!!glass:', !!glass);
console.log('!!glasses:', !!glasses);
console.log('!!watchGlass:', !!watchGlass);
console.groupEnd('데이터 -> 불리언 데이터로 변경되는 사례');
...
var eatLunch = new Function('console.log("와구 와구 먹는다.")'); // Function {}
// function(){}
...
console.log('convert_string_from_number:', convert_string_from_number);
...
// 아래 함수를 실행했을 때,
// 호이스트 현상이 발생하여 doYourself()의 경우는 실행되나,
...
// 1. 함수 선언문: 함수 이름 선언
// function doYourself(){} 몸체가 모두 영역 최상위로 끌어올려진다.
function doYourself(){
...
// init_style_of_body[font-size] 순환형에 

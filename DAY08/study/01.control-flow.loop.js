...
if (is_opened) {
    // 열려져 있는 상태 -> 닫힌 상태
    // 상태 변경
    is_opened = false; // var가 나왔을때 호이스팅 유의할 것(최상단으로 var is_opened가 올라가 undefined가 된다)
    // is_opened = !is_opened;
} else {
    // 닫힌 상태 -> 열린 상태
    is_opened = true;
...
var music_player_state = 'naviation previous song';
...
if ( music_player_state === 'play') {
    console.log('music_player_state is: PLAY');
}
// 조건 1이 거짓이면, 조건 2를 확인
else if ( music_player_state === 'pause') {
    console.log('music_player_state is: PAUSE');
}
// 조건 2가 거짓이면, 조건 3을 확인
else if ( music_player_state === 'stop') {
    console.log('music_player_state is: STOP');
} else { // 조건 1, 2, 3 모두 거짓이면 실행
    console.log('music_player_state is: UNKNOWN');
};
...
//     // console.error()와 달리 throw는 구문을 중단한다.
//     // console.error(error.message); // 오류를 잡아서 오류 메세지 출력
//    throw error.message;
...
for (var member in classUsingObject) {
    ...
    console.log('classUsingObject 객체의 속성:', prop);
}
